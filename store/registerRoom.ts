...
const registerRoom = createSlice({
  ...
  reducers: {
    ...
    setBedTypeCount(
      ...
      const preBeds = bedroom.beds;
      const index = preBeds.findIndex((bed) => bed.type === type);
      //* 타입이 없다면
      if (index == -1) {
        state.bedList[bedroomId - 1].beds = [...preBeds, { type, count }];
        return state;
      }
      //* 타입이 존재한다면
      if (count === 0) {
    ...
    setConveniences(state, action: PayloadAction<string[]>) {
      state.conveiences = action.payload;
    },
    //* 숙소 사진 변경하기
